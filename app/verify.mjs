  ...
  await page.route("https://flare-api.flare.network/**", async (route) => {
    ...
    if (request.method === "eth_call") {
      ...
      if (selector === selectors.isAuthorized) {
        result = encodeFunctionResult({ abi: morphoAbi, functionName: "isAuthorized", result: state.authorized });
      } else if (selector === selectors.policyOf) {
        result = encodeFunctionResult({ abi: managerAbi, functionName: "policyOf", result: policyResult(state.policy) });
      ...
      if (status === "0x1" && !state.appliedReceipts.has(hash)) {
        ...
        if (hash === HASHES.authorize.toLowerCase()) state.authorized = true;
        if (hash === HASHES.policy.toLowerCase()) state.policy = {
        ...
        if (hash === HASHES.disable.toLowerCase()) state.policy.enabled = false;
        if (hash === HASHES.revoke.toLowerCase()) state.authorized = false;
      }
