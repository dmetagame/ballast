...
contract BallastManager is IMorphoFlashLoanCallback {
    ...
    constructor(address morpho, address adapter) {
    ...
    /// @dev Permissionless: anyone may call, but only within the borrower's policy bounds.
    function protect(address borrower, Id id) external nonReentrant {
        ...
        uint256 slip = (uint256(p.maxSlippageBps) * WAD) / MAX_BPS;
        uint256 repayAssets = HealthMath.repayToReachTarget(cv, debt, mp.lltv, p.targetHealth, slip);
